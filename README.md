...
- **AI face recognition** – InsightFace (ArcFace, 512-d) by default, with an
  automatic fallback to `face_recognition` (dlib, 128-d). Embeddings are
...
The InsightFace model pack (`buffalo_l`) is downloaded on first run and cached
in the `insightface_models` volume so subsequent starts are fast.
...
| `AUTO_MATCH_THRESHOLD`   | `0.85`                                | Auto-confirm above this cosine sim.      |
| `SUGGEST_THRESHOLD`      | `0.55`                                | Suggest between suggest–auto threshold.  |
